...
## Corpus Check
- 54 files · ~11,383 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 413 nodes · 734 edges · 38 communities (30 shown, 8 thin omitted)
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 161 edges (avg confidence: 0.6)
...
1. `Repository` - 40 edges
2. `ConversationOrchestrator` - 31 edges
3. `StaleGenerationError` - 25 edges
4. `Order` - 22 edges
5. `DomainValidationError` - 20 edges
6. `PosReceipt` - 20 edges
7. `VersionedModel` - 18 edges
8. `Menu` - 18 edges
9. `compilerOptions` - 16 edges
10. `OrderLineInput` - 16 edges
...
Cohesion: 0.11
Nodes (39): BaseModel, datetime, FailingProvider, Path, repository(), SlowProvider, test_interruption_cancels_active_generation(), test_pos_is_idempotent() (+31 more)

### Community 4 - "Repository"
Cohesion: 0.07
Nodes (26): DeclarativeBase, DomainEvent, Protocol, Order, PosReceipt, CircuitBreaker, DeepgramSpeechToTextProvider, GroqLanguageProvider (+18 more)
...
Cohesion: 0.24
Nodes (21): test_calculates_modifier_quantity_and_tax(), test_rejects_missing_required_modifier(), test_replaces_line_for_correction(), add_line(), calculate_line(), DomainValidationError, get_item(), money() (+13 more)

### Community 7 - "app.py"
Cohesion: 0.15
Nodes (11): BaseSettings, FastAPI, Path, test_mock_mode_order_flow_without_keys(), Settings, Kubernetica voice-agent backend., build_pipecat_pipeline(), pipecat_available() (+3 more)
...
## Knowledge Gaps
- **139 isolated node(s):** `eslintConfig`, `workspaceRoot`, `nextConfig`, `name`, `version` (+134 more)
...
- **Why does `ConversationOrchestrator` connect `models.py` to `Repository`, `DomainValidationError`?**
...
- **Are the 23 inferred relationships involving `ConversationOrchestrator` (e.g. with `FailingProvider` and `SlowProvider`) actually correct?**
  _`ConversationOrchestrator` has 23 INFERRED edges - model-reasoned connections that need verification._
- **Are the 22 inferred relationships involving `StaleGenerationError` (e.g. with `FailingProvider` and `SlowProvider`) actually correct?**
  _`StaleGenerationError` has 22 INFERRED edges - model-reasoned connections that need verification._
- **Are the 20 inferred relationships involving `Order` (e.g. with `test_calculates_modifier_quantity_and_tax()` and `test_rejects_missing_required_modifier()`) actually correct?**
  _`Order` has 20 INFERRED edges - model-reasoned connections that need verification._